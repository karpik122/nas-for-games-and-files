...
import { z } from "zod";
import type { SystemProvider } from "./system/system-provider.js";
...
const storageRoot = path.resolve(
  process.env.STORAGE_ROOT ?? path.resolve(process.cwd(), "../../dev-storage"),
);
const storageRealRoot = await realpath(storageRoot);
const gameServersRoot = path.resolve(
  process.env.GAME_SERVERS_ROOT ?? path.resolve(process.cwd(), "../../game-servers"),
);
const databasePath = path.resolve(
  process.env.DATABASE_PATH ?? path.resolve(process.cwd(), "../../data/karpiknas.sqlite"),
);

function selectSystemProvider(): SystemProvider {
...
const gameCommandBodySchema = z.object({
  command: z.string().trim().min(1).max(500).refine(
    (command) => !/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F\r\n]/u.test(command),
    "Komenda zawiera niedozwolone znaki.",
  ),
});
...
function sessionCookie(token: string, maxAge = sessionDurationSeconds) {
...
async function resolveStoragePath(relativePath: string) {
  const normalizedPath = relativePath.replaceAll("\\", "/").replace(/^\/+/, "");
  const absolutePath = path.resolve(storageRoot, normalizedPath);
  const realPath = await realpath(absolutePath);
  const isInsideStorage =
    realPath === storageRealRoot || realPath.startsWith(`${storageRealRoot}${path.sep}`);
...
app.get("/api/games/servers/minecraft/console/logs", protectedRoute, async (request, reply) => {
  ...
  if (!parsedQuery.success) return reply.code(400).send({ message: "Nieprawidłowa liczba linii logów." });
...
app.get("/api/games/servers/minecraft/files", protectedRoute, async (request, reply) => {
  ...
  if (!parsedQuery.success) return reply.code(400).send({ message: "Nieprawidłowa ścieżka." });
...
app.post("/api/games/servers/minecraft/plugins", protectedRoute, async (request, reply) => {
  ...
  try {
    ...
    if (!upload) return reply.code(400).send({ message: "Wybierz plik pluginu .jar." });
