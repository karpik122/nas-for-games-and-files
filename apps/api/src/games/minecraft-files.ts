...
export class MinecraftFiles {
  ...
  async list(relativePath = "") {
    ...
    const result = await Promise.all(visibleEntries.map(async (entry): Promise<MinecraftFileEntry> => {
  ...
  async savePlugin(filename: string, stream: Readable & { truncated?: boolean }) {
    ...
    try {
      ...
      if (error instanceof MinecraftFileError) throw error;
  ...
  private async ensureDataRoot() {
    await mkdir(this.dataRoot, { recursive: true });
    return realpath(this.dataRoot);
  }

  private async ensurePluginsRoot() {
  ...
  private normalize(relativePath: string) {
    return relativePath.replaceAll("\\", "/").replace(/^\/+/, "");
  }

  private async resolveExisting(relativePath: string) {
    const realRoot = await this.ensureDataRoot();
  ...
  private async resolveWritable(relativePath: string) {
    const realRoot = await this.ensureDataRoot();
    const normalizedPath = this.normalize(relativePath);
    ...
    if (!realParent) throw new MinecraftFileError("Katalog nadrzędny nie istnieje.", 404);
  ...
  private assertInside(realRoot: string, target: string) {
  ...
  private isEditable(filename: string) {
